...
if (isset($_POST['i_Name'])) {
    ...
    try {
        ...
        if ($connection->connect_errno != 0) {
            ...
            // mail
            $result = $connection->query("SELECT id FROM users WHERE mail='$mail1'");

            if (!$result) throw new Exception($connection->error);

            $c_mails = $result->num_rows;
            if ($c_mails > 0) {
                $OK = false;
                $_SESSION['err_mail'] = "Istnieje już konto przypisane do tego adresu e-mail!";
            }
            // phone
            $result = $connection->query("SELECT id FROM users WHERE phone='$phone'");

            if (!$result) throw new Exception($connection->error);

            $c_phones = $result->num_rows;
            if ($c_phones > 0) {
                $OK = false;
                $_SESSION['err_phone'] = "Istnieje już konto przypisane do tego telefonu!";
            }
            // pesel
            $result = $connection->query("SELECT id FROM users WHERE pesel='$pesel'");

            if (!$result) throw new Exception($connection->error);

            $c_pesel = $result->num_rows;
            if ($c_pesel > 0) {
                $OK = false;
                $_SESSION['err_pesel'] = "Istnieje już konto przypisane do tego PESELu!";
            }
            if ($OK == true) {
                if ($connection->query("INSERT INTO users VALUES (NULL, '$name', '$surname', '$pesel', '$phone', '$mail1', '$pass_hash')")) {
